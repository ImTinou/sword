...
-- Made by esore 2026
-- Upgraded by TinouHub v2 — Auto-Fake hook + Scanner (voir bas du fichier)
...
-- ============================================================================
--  UPGRADE v2 — Auto-Fake hook + Scanner  (TinouHub)
-- ============================================================================
local Players = game:GetService("Players")
local LP      = Players.LocalPlayer

-- ── Style helpers (réutilise le look du GUI existant) ───────────────────────
local function styledButton(parent, text, size, pos)
    local btn = Instance.new("ImageButton")
    btn.ImageTransparency = 1
    btn.BorderSizePixel = 0
    btn.AutoButtonColor = true
    btn.Size = size or UDim2.new(0, 121, 0, 29)
    if pos then btn.Position = pos end
    btn.BackgroundColor3 = Color3.fromRGB(90, 99, 109)
    btn.Parent = parent
    local c = Instance.new("UICorner") c.CornerRadius = UDim.new(0, 6) c.Parent = btn
    local s = Instance.new("UIStroke") s.Color = Color3.fromRGB(154, 154, 154) s.Parent = btn
    ...
    t.TextScaled = true
    t.TextWrapped = true
    t.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
    t.Parent = btn
    return btn, t
end

-- ── Fake un achat terminé pour un id, tous les types de signaux ─────────────
local function fakeFinish(id, kind)
    id = tonumber(id)
    if not id then return false end
    if kind == "Gamepass" then
        pcall(function() MarketplaceService:SignalPromptGamePassPurchaseFinished(LP, id, true) end)
    elseif kind == "Product" then
        pcall(function() MarketplaceService:SignalPromptProductPurchaseFinished(LP.UserId, id, true) end)
    elseif kind == "Asset" then
        pcall(function() MarketplaceService:SignalPromptPurchaseFinished(LP.UserId, id, true) end)
    else
        -- inconnu : on tente tout
        pcall(function() MarketplaceService:SignalPromptProductPurchaseFinished(LP.UserId, id, true) end)
        pcall(function() MarketplaceService:SignalPromptGamePassPurchaseFinished(LP, id, true) end)
        pcall(function() MarketplaceService:SignalPromptPurchaseFinished(LP.UserId, id, true) end)
    end
    print("[FAKE] finished:", kind or "All", id)
    return true
end

-- ── Découverte d'IDs (capture passive + scan actif) ─────────────────────────
local found = {}          -- clé "kind:id" -> true (dédup)
local scanRows = 0

local function resolveName(id, kind)
    local infoType = Enum.InfoType.Product
    if kind == "Gamepass" then infoType = Enum.InfoType.GamePass
    elseif kind == "Asset" then infoType = Enum.InfoType.Asset end
...
local addScanRow  -- fwd

local function captureId(id, kind)
    id = tonumber(id)
...
addScanRow = function(id, kind)
    scanRows = scanRows + 1
    local row = Instance.new("Frame")
    row.BackgroundTransparency = 0.75
    row.Size = UDim2.new(0, 464, 0, 40)
    row.BackgroundColor3 = Color3.fromRGB(90, 99, 109)
    row.LayoutOrder = scanRows
    row.Parent = scannerTabFrame
    local c = Instance.new("UICorner") c.CornerRadius = UDim.new(0, 6) c.Parent = row
    local s = Instance.new("UIStroke") s.Color = Color3.fromRGB(154, 154, 154) s.Parent = row

    local nameLbl = Instance.new("TextLabel")
    nameLbl.BackgroundTransparency = 1
    nameLbl.Size = UDim2.new(0, 300, 0, 14)
    nameLbl.Position = UDim2.new(0, 12, 0.28, 0)
    nameLbl.AnchorPoint = Vector2.new(0, 0.5)
    nameLbl.Text = kind.." #"..id.."  (...)"
    nameLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
    nameLbl.TextXAlignment = Enum.TextXAlignment.Left
    nameLbl.TextScaled = true
    nameLbl.FontFace = Font.new("rbxasset://fonts/families/Inconsolata.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
    nameLbl.Parent = row

    local idLbl = Instance.new("TextLabel")
    idLbl.BackgroundTransparency = 1
    idLbl.Size = UDim2.new(0, 300, 0, 12)
    idLbl.Position = UDim2.new(0, 12, 0.72, 0)
    idLbl.AnchorPoint = Vector2.new(0, 0.5)
    idLbl.Text = kind.." • "..id
    idLbl.TextColor3 = Color3.fromRGB(255, 201, 37)
    idLbl.TextXAlignment = Enum.TextXAlignment.Left
    idLbl.TextScaled = true
    idLbl.FontFace = Font.new("rbxasset://fonts/families/Inconsolata.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
    idLbl.Parent = row

    local fakeBtn = styledButton(row, "Fake Buy", UDim2.new(0, 90, 0, 26), UDim2.new(1, -100, 0.5, 0))
    fakeBtn.AnchorPoint = Vector2.new(0, 0.5)
    fakeBtn.MouseButton1Click:Connect(function()
        fakeFinish(id, kind)
        addLog(resolveName(id, kind), id, true)
    end)

    -- résout le nom en arrière-plan
    task.spawn(function()
        local n = resolveName(id, kind)
        pcall(function() nameLbl.Text = n end)
    end)
end

-- ── Scan actif : Value objects + getgc ──────────────────────────────────────
local function scanInstances()
    local roots = { game:GetService("ReplicatedStorage"), workspace, game:GetService("StarterGui") }
        ...
        pcall(function()
            for _, v in ipairs(root:GetDescendants()) do
                if v:IsA("NumberValue") or v:IsA("IntValue") then
                    ...
                    if v.Value > 0 then
                        if nm:find("gamepass") or nm:find("pass") then captureId(v.Value, "Gamepass")
                        elseif nm:find("product") or nm:find("dev") then captureId(v.Value, "Product") end
                    end
                end
            end
        end)
                ...
                pcall(function()
                    for k, val in pairs(obj) do
                        if type(val) == "number" and val > 1000 then
                            local kl = type(k) == "string" and string.lower(k) or ""
                            if kl:find("gamepass") or kl == "passid" then captureId(val, "Gamepass")
                            elseif kl:find("productid") or kl:find("devproduct") then captureId(val, "Product") end
                        end
                    end
                end)
            end
...
-- ── Bouton SCAN en haut du tab Scanner ──────────────────────────────────────
local scanBtn, scanBtnLbl = styledButton(scannerTabFrame, "🔍 SCAN GAME", UDim2.new(0, 464, 0, 30))
scanBtn.LayoutOrder = 0
scanBtn.MouseButton1Click:Connect(function()
    scanBtnLbl.Text = "Scanning..."
    task.spawn(function()
        scanInstances()
        scanGC()
        scanBtnLbl.Text = "🔍 SCAN GAME ("..scanRows.." trouvés)"
    end)
end)

-- ── Tab Action : toggle Auto-Fake + Fake direct ─────────────────────────────
local autoFakeEnabled = false
local autoBtn, autoLbl = styledButton(actionTabFrame, "Auto-Fake: OFF")
autoBtn.MouseButton1Click:Connect(function()
    autoFakeEnabled = not autoFakeEnabled
    autoLbl.Text = "Auto-Fake: "..(autoFakeEnabled and "ON ✓" or "OFF")
    autoBtn.BackgroundColor3 = autoFakeEnabled and Color3.fromRGB(104, 165, 123) or Color3.fromRGB(90, 99, 109)
end)

local fakeAllBtn = styledButton(actionTabFrame, "Fake ID (tous types)")
fakeAllBtn.MouseButton1Click:Connect(function()
    local id = tonumber(ProductIDInput.Text)
    if not id then warn("Invalid Product ID") return end
    fakeFinish(id, nil)
end)

-- ── Hook __namecall : intercepte les prompts d'achat ────────────────────────
-- Capture toujours (remplit le scanner). Si Auto-Fake ON : bloque le vrai prompt
-- et signale l'achat comme réussi → les jeux qui valident côté client se font duper.
if type(hookmetamethod) == "function" and type(getnamecallmethod) == "function" then
    ...
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if self == MarketplaceService then
            local args = {...}
            local kind, id
            if method == "PromptGamePassPurchase" then kind, id = "Gamepass", tonumber(args[2])
            elseif method == "PromptProductPurchase" then kind, id = "Product", tonumber(args[2])
            elseif method == "PromptPurchase" then kind, id = "Asset", tonumber(args[2]) end
            if id then
                task.spawn(function() captureId(id, kind) end)
                if autoFakeEnabled then
                    task.spawn(function() fakeFinish(id, kind) end)
                    return  -- bloque le vrai prompt Robux
                end
            end
        end
        return oldNamecall(self, ...)
    end)
    print("[TinouHub] fake.lua v2 — hook __namecall actif")
else
    warn("[TinouHub] hookmetamethod indisponible — Auto-Fake désactivé, boutons manuels OK")
end